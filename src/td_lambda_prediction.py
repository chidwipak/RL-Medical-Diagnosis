...
2. For each episode:
   ...
   - For each step (s, a, r, s'):
     ...
     - for all x: V(x) += alpha * delta * E(x)
     - for all x: E(x) = gamma * lambda * E(x)
"""

import numpy as np
from typing import List, Tuple

class TDLambdaPrediction:
    DISEASE_PATTERNS = {
    ...
    def __init__(self, gamma: float = 0.9, alpha: float = 0.05, lambd: float = 0.8):
        self.n_states = 244
        self.gamma = gamma
        self.alpha = alpha
        self.lambd = lambd
    ...
    def run(self, n_episodes: int = 10000):
        print("Starting TD(λ) Prediction (Random Policy)...")
        self.V = np.zeros(self.n_states)
        ...
        for ep in range(n_episodes):
            ...
            for _ in range(15):
                ...
                # Update all states (vectorized for speed)
...
if __name__ == "__main__":
    td_lambda = TDLambdaPrediction(alpha=0.05, lambd=0.8)
    td_lambda.run(10000)
